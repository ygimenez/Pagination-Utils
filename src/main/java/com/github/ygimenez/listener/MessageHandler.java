...
public class MessageHandler extends ListenerAdapter {
	...
	private String getEventId(GenericMessageEvent evt) {
		crc.reset();
		String rawId = (evt.isFromGuild() ? "GUILD_" + evt.getGuild().getId() : "PRIVATE_" + evt.getChannel().getId()) + "_" + evt.getMessageId();
	...
	private String getEventId(ButtonInteractionEvent evt) {
		crc.reset();
		String rawId = (evt.getGuild() != null ? "GUILD_" + evt.getGuild().getId() : "PRIVATE_" + evt.getChannel().getId()) + "_" + evt.getMessageId();
		crc.update(rawId.getBytes(StandardCharsets.UTF_8));
	...
	private String getEventId(Message msg) {
		crc.reset();
		String rawId = (msg.isFromGuild() ? "GUILD_" + msg.getGuild().getId() : "PRIVATE_" + msg.getChannel().getId()) + "_" + msg.getId();
		crc.update(rawId.getBytes(StandardCharsets.UTF_8));
